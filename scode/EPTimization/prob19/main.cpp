...
ll fh(int k ){
  ...
  f(i,q+1,14){
    f(j,i+1,14){
      if(((k&(1<<i))!=0) && ((k&(1<<j))!=0) && p[i].second == p[q].second && p[q].first!=p[i].first && p[q].first!=p[j].first && p[q].second==p[j].second){
        dp[k] = max(dp[k],fh(k-(1<<i)-(1<<j)-(1<<q))+m[p[q].second]*3);
      }
    }
  }
  f(i,q+1,14){
    f(j,i+1,14){
      f(h,j+1,14){
        if(((k&(1<<i))!=0) && ((k&(1<<j))!=0) && ((k&(1<<h))!=0) && p[h].second==p[q].second && p[h].first!=p[q].first&& p[i].second == p[q].second && p[q].first!=p[i].first && p[q].first!=p[j].first && p[q].second==p[j].second){
          dp[k] = max(dp[k],fh(k-(1<<i)-(1<<j)-(1<<q)-(1<<h))+m[p[q].second]*4);
        }
      }
    }
  }
...
void solve()
{
  ...
  f(i,0,14){
    ...
    p[i].second =t;
    ma[p[i].first].pb(m[p[i].second]);
  }
  int k = fh((1<<14)-1);
    ll sum = 0,sum1=0;
  for(auto x: ma){
    for(int i = 0;i<(x.second).size();i++){
      if(x.second.size()>3)sum1+=x.second[i];
      sum+=x.second[i];
    }
  }
  if(sum1<72){
