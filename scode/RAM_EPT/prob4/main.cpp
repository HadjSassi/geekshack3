...
void solve()
{
    ...
    cin>>l>>h>>x>>y>>vx>>vy>>K;
    ll i;
    pair<ld,ld> inter1, inter2;
    f(0,K,1) {
        if(vx > 0 && vy > 0) {
            inter1 = make_pair((vx/vy)*(h-y)+x,h);
            inter2 = make_pair(l,(vy/vx)*(l-x)+y);
            if(inter1.first < l) {x = inter1.first;y=inter1.second; vy = -vy;}
            else if(inter1.first > l) {x = inter2.first;y=inter2.second; vx = -vx;}
            else {x = inter1.first;y=inter1.second; vy = -vy; vx = -vx;}
            //cout<<i<<" : "<<x<<" "<<y<<endl;
        }
        else if(vx > 0 && vy < 0) {
            inter1 = make_pair((vx/vy)*(0-y)+x,0);
            inter2 = make_pair(l,(vy/vx)*(l-x)+y);
            if(inter1.first < l) {x = inter1.first;y=inter1.second; vy = -vy;}
            else if(inter1.first > l) {x = inter2.first;y=inter2.second; vx = -vx;}
            else {x = inter1.first;y=inter1.second; vy = -vy; vx = -vx;}
            ...
            inter1 = make_pair((vx/vy)*(h-y)+x,h);
            inter2 = make_pair(0,(vy/vx)*(0-x)+y);
            if(inter1.first > 0) {x = inter1.first;y=inter1.second; vy = -vy;}
            else if(inter1.first < 0) {x = inter2.first;y=inter2.second; vx = -vx;}
            else {x = inter1.first;y=inter1.second; vy = -vy; vx = -vx;}
            //cout<<i<<" : "<<x<<" "<<y<<endl;
        }
        else if(vx < 0 && vy < 0) {
